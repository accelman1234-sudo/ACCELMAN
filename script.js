...
function agregarProducto() {
    ...
    const urlFoto = document.getElementById("urlFoto") ? document.getElementById("urlFoto").value.trim() : "";

    if(!modelo || !precio || !categoria || (indiceEditar === null && !urlFoto && fotoInput.files.length === 0)){
    ...
    reader.onload = () => {
        const producto = {
            ...
            descripcion,
            categoria,
            foto: urlFoto || (fotoInput.files.length > 0 ? reader.result : indiceEditar !== null ? productos[indiceEditar].foto : "")
        };
...
function limpiarFormulario() {
    ...
    if(document.getElementById("urlFoto")) document.getElementById("urlFoto").value = "";
}
...
// Mostrar productos cliente + fotos GitHub
function mostrarProductosCliente() {
    productosCliente.innerHTML = "";

    let todosProductos = [...productos];

    // Agregamos fotos GitHub como productos ficticios
    imagenesGitHub.forEach((url, i) => {
        todosProductos.push({
            modelo: `Foto negocio ${i+1}`,
            precio: "",
            descripcion: "",
            categoria: "Galería",
            foto: url
    ...
    let categorias = ["Celulares","Cargadores","Fundas","Accesorios","Galería"];
    if(filtroCategoriaCliente) categorias = [filtroCategoriaCliente];

    categorias.forEach(cat => {
        let productosCat = todosProductos.filter(p => p.categoria === cat);
        if(busquedaCliente) productosCat = productosCat.filter(p => p.modelo.toLowerCase().includes(busquedaCliente));
        productosCat.sort((a,b) => ordenPrecioCliente ? (b.precio || 0) - (a.precio || 0) : (a.precio || 0) - (b.precio || 0));
        if(productosCat.length === 0) return;
        ...
        productosCat.forEach(p => {
            ...
            card.innerHTML = `
                ...
                ${p.categoria !== "Galería" ? `<button class="btn-whatsapp" onclick="enviarWhatsApp('${p.modelo}','${p.foto}')">📲 WhatsApp</button>` : ""}
            `;
            grid.appendChild(card);
        });
...
function agregarProductoDesdeGitHub(url) {
    const modelo = prompt("Ingresa el nombre del producto:", "Producto GitHub");
    const precio = prompt("Ingresa el precio:", "0");
    const categoria = prompt("Ingresa la categoría (Celulares, Cargadores, Fundas, Accesorios):", "Accesorios");
    const descripcion = prompt("Ingresa descripción (opcional):", "");
