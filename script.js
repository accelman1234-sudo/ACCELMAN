...
document.getElementById("btnGitHub").addEventListener("click", () => {
    ...
    divForm.innerHTML = `
        <input type="text" id="urlGitHubManual" placeholder="Pega la URL de la imagen aquí" style="width:100%;">
        <input type="text" id="modeloGitHub" placeholder="Nombre del producto" style="width:100%;">
        <input type="number" id="precioGitHub" placeholder="Precio" style="width:100%;">
        <select id="categoriaGitHub" style="width:100%;">
        ...
        <textarea id="descripcionGitHub" placeholder="Descripción (opcional)" style="width:100%;"></textarea>
        <button id="btnAgregarURLGitHub">Agregar Producto</button>
    `;
    lista.appendChild(divForm);

    document.getElementById("btnAgregarURLGitHub").addEventListener("click", () => {
        const url = document.getElementById("urlGitHubManual").value.trim();
        ...
        const githubBase = "https://raw.githubusercontent.com/accelman1234-sudo/ACCELMAN/";
        if (!url.startsWith(githubBase)) {
            alert("Solo se permiten imágenes de tu repositorio de GitHub");
            return;
        }
        ...
        const producto = { modelo, precio, categoria, descripcion, foto: url };
        productos.push(producto);
        localStorage.setItem("productos", JSON.stringify(productos));
        mostrarProductosAdmin();
        mostrarProductosCliente();
